...
class DDPG:
    ...
    def take_action(self, state):# state(4,n)----state(1,4)
        state = torch.tensor([state], dtype=torch.float).to(self.device)
        action = self.actor(state).detach().cpu().numpy()[0]# detach() 的作用---从计算图中分离张量
        # 给动作添加噪声，增加探索
...
class ReplayBuffer_Trajectory:
    ...
    def sample(self, batch_size, use_her, dis_threshold=0.15, her_ratio=0.8):
        ...
        for _ in range(batch_size):
            traj = random.sample(self.buffer, 1)[0]# 随机取一条轨迹
            ...
            if use_her and np.random.uniform() <= her_ratio:# 使用事后经验回放（HER），使用 HER 的概率，默认为 0.8。
                step_goal = np.random.randint(step_state + 1, traj.length + 1)# 设置目标状态
                goal = traj.states[step_goal][:2]  # 使用HER算法的future方案设置目标
                dis = np.sqrt(np.sum(np.square(next_state[:2] - goal)))
                reward = -1.0 if dis > dis_threshold else 0# 计算距离并设置奖励
                done = False if dis > dis_threshold else True
                state = np.hstack((state[:2], goal))# 更新状态
...
for i in range(10):
    with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
        for i_episode in range(int(num_episodes / 10)):
            ...
            while not done:# 每个Episode中智能体的一整条轨迹存储到traj中
                action = agent.take_action(state)
                state, reward, done = env.step(action)
                episode_return += reward
                traj.store_step(action, state, reward, done)
            replay_buffer.add_trajectory(traj)# 将轨迹存储到经验回放区中
            return_list.append(episode_return)
            if replay_buffer.size() >= minimal_episodes:# 经验回放区数据大于一定条件后执行更新
